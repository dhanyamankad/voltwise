...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const theme = {
        default: {
          bg: "#0D1117",
          lineActive: { r: 79, g: 193, b: 201, a: 0.9 },
          nodeActive: { r: 242, g: 169, b: 59, a: 1.0 },
          glow: "79,193,201",
          ripple: "79,193,201",
        },
        monochrome: {
  ...
  useEffect(() => {
    ...
    const onCustomRipple = (e: Event) => {
      const customEv = e as CustomEvent<{ x?: number; y?: number }>;
      const x = customEv.detail?.x ?? window.innerWidth * 0.5;
      const y = customEv.detail?.y ?? window.innerHeight * 0.35;
      const now = performance.now();
      ripplesRef.current.push(
        { x, y, radius: 0, opacity: 1, born: now },
        { x, y, radius: -60, opacity: 0.85, born: now + 140 },
        { x, y, radius: -120, opacity: 0.6, born: now + 280 }
      );
    };

    window.addEventListener("mousemove", onMouseMove);
    window.addEventListener("click", onClick);
    window.addEventListener("voltwise:ripple", onCustomRipple);
    rafRef.current = requestAnimationFrame(animate);
    ...
    return () => {
      ...
      window.removeEventListener("voltwise:ripple", onCustomRipple);
      if (rafRef.current) {
  ...
  return (
    <div
      className={cn(
        ...
        globalColor === "monochrome" ? "bg-[#000000]" : "bg-[#0D1117]",
        className,
      )}
    >
      <canvas
